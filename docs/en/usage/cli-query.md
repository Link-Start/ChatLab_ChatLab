...
`chatlab-analyze` always remains read-only. To let an agent import a new chat export, use the separate `chatlab-import` skill; it previews the write and waits for confirmation first. See the [Import Chat Records Guide](./how-to-import.md).
